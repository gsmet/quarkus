        ...
        cc.method("equals", mc -> {
            ...
            mc.body(bc -> {
                ...
                for (MethodInfo annotationMember : literal.annotationMembers()) {
                    ...
                    InstanceFieldVar thisValue = cc.this_().field(field);

                    // for the other object, must invoke the method
                    LocalVar thatValue = bc.localVar("thatValue", bc.invokeInterface(methodDescOf(annotationMember), that));
                    ...
                    if (type.equals(ConstantDescs.CD_float)) {
                        ...
                        cmp = bc.objEquals(thisValue, thatValue);
                    }
    ...
    private static void generateHashCode(ClassCreator cc, AnnotationLiteralClassInfo literal) {
        cc.method("hashCode", mc -> {
            ...
            mc.body(bc -> {
                ...
                for (MethodInfo annotationMember : literal.annotationMembers()) {
                    Expr memberNameHash = bc.mul(Const.of(127), bc.objHashCode(Const.of(annotationMember.name())));

                    ClassDesc type = classDescOf(annotationMember.returnType());
                    FieldDesc field = FieldDesc.of(cc.type(), annotationMember.name(), type);
                    InstanceFieldVar value = cc.this_().field(field);
                    Expr memberValueHash = type.isArray() ? bc.arrayHashCode(value) : bc.objHashCode(value);
        ...
        cc.method("toString", mc -> {
            ...
            mc.body(bc -> {
                ...
                for (MethodInfo annotationMember : literal.annotationMembers()) {
                    ...
                    FieldDesc field = FieldDesc.of(cc.type(), annotationMember.name(), type);
                    InstanceFieldVar value = cc.this_().field(field);
                    str.append(type.isArray() ? bc.arrayToString(value) : bc.objToString(value));
