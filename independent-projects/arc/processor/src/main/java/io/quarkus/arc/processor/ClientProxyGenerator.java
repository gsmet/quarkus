        ...
        gizmo.class_(generatedName, cc -> {
            ...
            cc.constructor(mc -> {
                ...
                mc.body(bc -> {
                    ...
                    LocalVar arc = bc.localVar("arc", bc.invokeStatic(MethodDescs.ARC_CONTAINER));
            ...
            for (MethodInfo method : getDelegatingMethods(bean, bytecodeTransformerConsumer, transformUnproxyableClasses)) {
                ...
                cc.method(method.name(), mc -> {
                    // TODO the Gizmo 1 version used to produce a generic signature that was different from
                    //  what we produce here, because it substituted certain type variables, but it also was
                    //  _incorrect_ (didn't match the descriptors at all when substitution happened)
                    ...
                    for (int i = 0; i < method.parametersCount(); i++) {
                        params.add(mc.parameter("param" + i, genericTypeOf(method.parameterType(i))));
                    }
                    ...
                    mc.body(bc -> {
                        if (!isInterface) {
                            ...
                            bc.ifNull(cc.this_().field(beanField), b1 -> {
                                if (method.isAbstract()) {
                                    ...
                                    MethodDesc superDesc = ClassMethodDesc.of(superClass, method.name(),
                        ...
                        LocalVar delegate = bc.localVar("delegate", bc.invokeVirtual(delegateMethod, cc.this_()));
                        ...
                        if (Methods.isObjectToString(method)) {
                            ...
                            ret = bc.invokeVirtual(originalMethodDesc, delegate, params);
                        } else if (isInterface) {
                            ...
                            ret = bc.invokeInterface(virtualMethod, delegate, params);
                            ...
                            MethodDesc virtualMethod = ClassMethodDesc.of(providerClassDesc,
                                    originalMethodDesc.name(), originalMethodDesc.type());
                            ret = bc.invokeVirtual(virtualMethod, delegate, params);
                        }
                        bc.return_(ret);
                    });
                });
