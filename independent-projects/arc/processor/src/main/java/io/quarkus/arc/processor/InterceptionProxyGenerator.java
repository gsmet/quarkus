...
public class InterceptionProxyGenerator extends AbstractGenerator {
    ...
    private void createInterceptionSubclass(Gizmo gizmo, InterceptionProxyInfo interceptionProxy,
        ...
        boolean isInterface = pseudoBean.getImplClazz().isInterface();

        CodeGenInfo info = preprocess(pseudoBean);

        ClassDesc superClass = isInterface ? ConstantDescs.CD_Object : pseudoBeanClass;
            ...
            for (MethodInfo method : methodsToForward) {
                ...
                cc.method(methodDesc, mc -> {
                    ...
                    List<ParamVar> params = IntStream.range(0, method.parametersCount())
                            .mapToObj(i -> mc.parameter("param" + i))
                            .toList();
                    mc.body(bc -> {
                        bc.return_(method.declaringClass().isInterface()
                                ? bc.invokeInterface(methodDesc, cc.this_().field(delegateField), params)
                                : bc.invokeVirtual(methodDesc, cc.this_().field(delegateField), params));
                    });
                });
            }
        });
