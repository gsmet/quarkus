...
public class InterceptedStaticMethodsProcessor {
        ...
        return cc.staticMethod("init_" + btMethod.name() + "_" + interceptedStaticMethod.getHash(), mc -> {
            ...
            mc.body(b0 -> {
                ...
                LocalVar rtMethod = b0.localVar("method", b0.blockExpr(Reflection2Gizmo.classDescOf(Method.class), b1 -> {
                    ...
                    b1.yield(b1.invokeStatic(MethodDescs.REFLECTIONS_FIND_METHOD, className, name, params));
                }));
                ...
                Expr metadata = b0.new_(MethodDescs.INTERCEPTED_METHOD_METADATA_CONSTRUCTOR,
    ...
    private Expr createInterceptorInvocation(InterceptorInfo interceptor, BlockCreator bc,
        ...
        Expr arc = bc.invokeStatic(MethodDescs.ARC_CONTAINER);
        Expr bean = bc.invokeInterface(MethodDescs.ARC_CONTAINER_BEAN, arc, Const.of(interceptor.getIdentifier()));
        LocalVar interceptorBean = bc.localVar("interceptor", bc.cast(bean, InjectableInterceptor.class));
        Expr creationalContext = bc.invokeStatic(MethodDescs.CREATIONAL_CTX_CHILD, parentCreationalContext);
        Expr interceptorInstance = bc.invokeInterface(MethodDescs.INJECTABLE_REF_PROVIDER_GET, interceptorBean,
                creationalContext);
        return bc.invokeStatic(MethodDescs.INTERCEPTOR_INVOCATION_AROUND_INVOKE, interceptorBean, interceptorInstance);
    }
