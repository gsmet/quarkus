...
public @interface Urgent {
    ...
    /**
     * Supports inline instantiation of the qualifier.
     */
...
When CDI injection is not available (e.g. in a static method or a class that is not a CDI bean), a `Signal` instance can be created programmatically using the `Signal.create()` factory method:

[source,java]
----
Signal<OrderPlaced> orderPlaced = Signal.create(OrderPlaced.class);

// With qualifiers
Signal<OrderPlaced> urgentOrderPlaced = Signal.create(OrderPlaced.class, Urgent.Literal.INSTANCE);
----
...
[source,java]
----
Signal<List<OrderPlaced>> orderPlaced = Signal.create(new TypeLiteral<List<OrderPlaced>>() {});
----
...
The `Uni` returned by `reactive().publish()`, `reactive().send()`, and `reactive().request()` is _lazy_: no signal is emitted until the `Uni` is subscribed.
